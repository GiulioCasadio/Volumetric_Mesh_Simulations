...
public class AbstractMesh : MonoBehaviour
{
    ...
    protected void PlaneSlice(float a, float b, float c, float d, float dim)
    {
        ...
        foreach (Vector3 centroid in polyCentroids)
        {
            if (this.gameObject.GetComponent<AbstractMesh>().oldvisiblePolys[i] == true)
                this.gameObject.GetComponent<AbstractMesh>().visiblePolys[i] = true;
            else
                this.gameObject.GetComponent<AbstractMesh>().visiblePolys[i] = false;
            i++;
        }*/
    }
    ...
    protected void SplitMesh(GameObject current)
    {
       ...
       /* if (current.name.Equals("firstSon"))
        {
            sum = 0;
        }
        ...
        */
        ...
        export_submesh(current, 0, upper);
        export_submesh(current, 1, lower);
        ...
        if (transform.name.Equals("Fruit"))
        {
            upper.AddComponent<StillPosition>();
            lower.AddComponent<StillPosition>();
        }

        // Aggiungo una forza alle mesh
        if (transform.name.Equals("Fruit") && !GameObject.Find("Throwers"))
        {
            upper.GetComponent<Rigidbody>().velocity = current.GetComponent<Rigidbody>().velocity;
            lower.GetComponent<Rigidbody>().velocity = current.GetComponent<Rigidbody>().velocity;
            upper.GetComponent<Rigidbody>().AddForce(GameObject.Find("Player").transform.forward *  3000);
            lower.GetComponent<Rigidbody>().AddForce(GameObject.Find("Player").transform.forward * -3000);
        }
        else if (transform.name.Equals("Fruit"))
        {
            upper.GetComponent<Rigidbody>().mass = lower.GetComponent<Rigidbody>().mass = 1;
            Transform player = GameObject.Find("Player").transform;
            upper.GetComponent<Rigidbody>().AddForce(player.Find("LatoSx").transform.forward * 250);
            lower.GetComponent<Rigidbody>().AddForce(player.Find("LatoDx").transform.forward * 250);
        }
        else if (transform.name.Equals("Urn"))
        {
            print("wwwww");
            upper.GetComponent<Rigidbody>().velocity = lower.GetComponent<Rigidbody>().velocity = current.GetComponent<Rigidbody>().velocity;
    ...
    void export_submesh(GameObject input_mesh, int label_val, GameObject output_mesh)
    {
